/**
 * Sector ETF session % — derived from live desk symbols (multi-leg betas).
 * Yahoo ETF quotes merged when API bundle provides them.
 */
...
export const SECTOR_FLOW_MODEL = [
  ...
  { ticker: 'XLV', name: 'Healthcare', legs: [{ s: 'ESUSD', w: 0.52 }, { s: 'NQUSD', w: 0.06 }] },
  { ticker: 'XLP', name: 'Consumer Staples', legs: [{ s: 'ESUSD', w: 0.42 }, { s: 'XAUUSD', w: 0.08 }] },
  { ticker: 'XLU', name: 'Utilities', legs: [{ s: 'ESUSD', w: 0.38 }, { s: 'CLUSD', w: -0.06 }] },
];
...
/** Yahoo rows fill gaps; live tape wins when it has stronger session signal. */
...
export function mergeFlowRowsLive(yahooRows = [], tapeRows = []) {
  ...
  for (const row of yahooRows) {
    ...
    const yPct = Number(row.pct);
    const tPct = Number(existing?.pct ?? 0);
    const yAbs = Math.abs(yPct);
    const tAbs = Math.abs(tPct);
    // Prefer Yahoo ETF when it has a real session move; never overwrite tape with flat Yahoo zero.
    if (!existing) {
...
export function flowSourceLabel(source) {
  const s = String(source || '');
  if (s.includes('yahoo') && s.includes('tape')) return 'Live ETF + tape';
  if (s.includes('yahoo')) return 'Live ETF';
  if (s === 'live_tape') return 'Live tape';
  if (s.includes('awaiting') || s.includes('flat')) return 'Awaiting tape';
  return 'Session %';
}
